...
class EventsIndex
  ...
  SORTABLE_FIELDS = [:title, :updated_at, :created_at, :date]
  PER_PAGE = 10

  delegate :params,   to: :controller
  delegate :events_url, to: :controller
  ...
  def events
    @events       ||= Event.fetch_all(options_filter)
    @events_items ||= @events.order(sort_params)
                         .paginate(page: current_page, per_page: per_page)
  end

  def total_items
    @total_items ||= @events.size
  end

  def links
    {
      first: events_url(rebuild_params.merge(first_page)),
      prev:  events_url(rebuild_params.merge(prev_page)),
      next:  events_url(rebuild_params.merge(next_page)),
      last:  events_url(rebuild_params.merge(last_page))
    }
  end
  ...
  private
    ...
    def options_filter
      params.permit('id', 'title', 'date', 'search', 'sort', 'event', 'event' => {}).tap do |filter_params|
        filter_params[:page]= {}
