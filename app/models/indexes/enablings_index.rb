...
class EnablingsIndex
  ...
  def enablings
    @enablings       ||= Enabling.fetch_all(options_filter)
    @enablings_items ||= @enablings.order(sort_params)
                                   .paginate(page: current_page, per_page: per_page)
  end

  def total_items
    @total_items ||= @enablings.size
  end

  def links
    {
      first: enablings_url(rebuild_params.merge(first_page)),
      prev:  enablings_url(rebuild_params.merge(prev_page)),
      next:  enablings_url(rebuild_params.merge(next_page)),
      last:  enablings_url(rebuild_params.merge(last_page))
    }
  end
  ...
  private
    ...
    def options_filter
      params.permit('id', 'search', 'name', 'sort', 'enabling', 'enabling' => {}).tap do |filter_params|
        filter_params[:page]= {}
    ...
    def total_pages
      @total_pages ||= enablings.total_pages
    end
