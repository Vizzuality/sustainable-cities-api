...
class BlogsIndex
  ...
  delegate :blogs_url, to: :controller
  ...
  def blogs
    @blogs       ||= Blog.fetch_all(options_filter)
    @blogs_items ||= @blogs.order(sort_params)
                         .paginate(page: current_page, per_page: per_page)
  end

  def total_items
    @total_items ||= @blogs.size
  end

  def links
    {
      first: blogs_url(rebuild_params.merge(first_page)),
      prev:  blogs_url(rebuild_params.merge(prev_page)),
      next:  blogs_url(rebuild_params.merge(next_page)),
      last:  blogs_url(rebuild_params.merge(last_page))
    }
  end
  ...
  private
    ...
    def options_filter
      params.permit('id', 'title', 'date', 'search', 'sort', 'blog', 'blog' => {}).tap do |filter_params|
        filter_params[:page]= {}
